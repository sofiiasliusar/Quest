# Коментарі до нових команд

import random # включає модуль random, який дозволяє генерувати випадкові числа або обирати випадкові елементи
import time #додається модуль time, що дає можливість працюваати з часом (наприклад, виконувати затримку виконання програми)

def print_with_pause(text): 
# def - блок коду, що виконує певне завдання. Йому надається ім'я, яке мможна викликати у програмі
# def функція (параметри):
# Тіло функції
# return - результат (необов'язково виводити)
    print(text) # друкує текст, переданий в фунцію print_with_pause
    time.sleep(1) #затримує виконання команди на 1 секунду

class Player: #клас (шаблон для визначення об'єкта зі змінними атрибутами та методами, функціями. Таким чином створюємо структуру, яка може бути використана для різних гравців з різними характеристиками)
    def __init__(self, name):
# __init__це конструктор, який викликається при створенні нового об'єкта класу для ініціалізації атрибутів об'єкта.
#  Цей метод приймає принаймні один аргумент, зазвичай названий "self", 
# який посилається на сам об'єкт, який створюється. 

        self.name = name # self параметр, який дає доступ до атрибутів
        self.health = 100 # name, health, attack - атрибути, які створюють спільні характеристики для майбутніх об'єктів цього класу
        self.attack = 10 # name, 10,100 - початкові значення, які присвоюються створеним гравцям

    def is_alive(self): #метод класу з ім'ям is_alive, що приймає один параметр self
        return self.health > 0 #якщо self.health > 0 повертається True і гравець живий, якщо ні, то False і гравець мертвий

    def take_damage(self, damage): # метод із назвою take_damage
# damage - параметр, що передається методу, і вказує на кількість пошкоджень, які гравець отримує.
        self.health -= damage 
# ця лінія зменшує життєву енергію гравця на кількість, вказану у параметрі damage. Вона віднімає damage від поточного рівня життєвої енергії гравця.

    def attack_enemy(self, enemy): # для атаки ворога
        damage = random.randint(0, self.attack) 
# генерує випадковий рівень пошкоджень, використовуючи значення сили атаки гравця (self.attack). 
# значення пошкоджень виражається як випадкове ціле число в діапазоні від 0 до self.attack.

        enemy.take_damage(damage)
# викликає метод take_damage для enemy, передаючи йому випадкове значення пошкоджень damage. 
# це означає, що атака гравця завдає пошкоджень іншому гравцю.
...
def intro():
    print_with_pause("Вітаю у грі!") #друкує текст, переданий у функцію і чекає секунду
...
def first_choice(): 
#  Функція, для групування коду та виконання певного завдань. У цьому випадку функція призначена для вибору одного з трьох варіантів, які представлені користувачу.
    choice1 = input("1, 2, або 3? ") #параметр
    if choice1 == "1": #порівнює і визначає, яку функцію викликати далі
        underwater_world()
    elif choice1 == "2":
        magical_forest()
    elif choice1 == "3":
        ...
        first_choice() #рекурсивний виклик функціїї (сама себе кличе)
...
# Створюємо новий об'єкт player класу Player
# першому атрибуту name присвоюємо значення (параметру у другому рядку та аргументу у третьому) player_name (ввід користувача)


    enemies = [Enemy("Мокра Курка", 20, 5), Enemy("Лохнеське Чудовисько", 30, 8), Enemy("Вужик", 50, 15)] 
# список з класів, яким присвоюються значення для атрибутів name, health, attack
    enemy = random.choice(enemies)
# випадковий вибір ворога зі списку
    print_with_pause(f"З'являється серйозно налаштований суперник - {enemy.name} !\n")

    while enemy.is_alive():
# зовнішній цикл, який продовжується доти, доки суперник (enemy) живий
        
        while enemy.is_alive() and player.is_alive():
# вкладений цикл, який виконується, доки обидва гравці (гравець і суперник) живі
            action = input("Що будете робити? \n Бити ногами (1), \n Не бити ногами (2)? : ")
            
            if action == "1":
                # Якщо користувач обирає "1", то гравець атакує суперника
                player.attack_enemy(enemy) 
                # викликається метод attack_enemy(enemy) для гравця
                if enemy.is_alive():
                    enemy.attack_player(player)
                # якщо суперник залишається живим, то суперник атакує гравця.
            elif action == "2":
                print_with_pause("Ви зробили правильний вибір і зекономили собі життєву енергію.")
                # виводиться повідомлення про правильний вибір
                break
                # користувач  ухиляється від атаки і програма виходить з вкладеної петлі

            else:
                print_with_pause("Будьте серйозні, будь ласка!")

# Після завершення вкладеної петлі перевіряється стан гравця (player) 
# та стан суперника (enemy) за допомогою методів is_alive(). 
# Якщо гравець не живий (not player.is_alive()), 
# гравця вводиться в тюрму з допомогою функції in_prison(). 
# Якщо суперник не живий (not enemy.is_alive()), гравець переміг, і виводиться відповідне повідомлення. 
# Якщо жоден з цих випадків не виконується, гра продовжується, і користувачу пропонується продовжити бій.
...
def fight_dog():
    ...
    if action == "1":
        ...
        if random.randint(0, 1) == 1:
            # random генерує випадкове число між 0 і 1
            # якщо випадає 1, то гравець перемагає
            # якщо випадає 0, то гравець цей раунд програє
...
def shoot_penalty():
    ...
    # випадковий вибір зі списку голів

    print(f"Ви б'єте {player_choice}, і Роналду робить стрибок {goalkeeper_choice}.")
    
    if player_choice == goalkeeper_choice: 
    # порівняння вибору і якщо збігається, то воротар ловить 
        print("Роналду зловив!")
        main_game()
    else:
        print("ГООООООООООЛ!!!")
    # порівняння вибору і якщо не збігається, то воротар не ловить 
        your_victory()
...
def hello_kitty():
    ...
    # список з передбаченнями
    def get_random_fortune():
        return random.choice(fortunes) # вибрати випадковий елемент зі списку fortunes і повернути його як результат
    while True: # безперервний цикл, бо True завжди виконується
...
def fight_dragon():
    # використання того ж класу Player зі зміненими іменами та діями
    player_name = input("Виберіть собі потужний псевдонім: ")
    player = Player(player_name)
